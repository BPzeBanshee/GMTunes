...
for (var xx = 0; xx < ww; xx+=16)
	...
	for (var yy = 0; yy < hh; yy+=16)
		...
		if data > 0 then draw_sprite(spr_note_ctrl,data-1,xx,yy);
		}
	}
...
if data > 0 then draw_sprite(spr_note_ctrl,data-1,rx,ry);
surface_reset_target();		
}
update_surf = function(){
var ww = 160;//ds_grid_width(global.pixel_grid);
var hh = 104;//ds_grid_height(global.pixel_grid);
if !surface_exists(pixel_surf) then pixel_surf = surface_create(ww,hh);
...
for (var xx = 0; xx < ww; xx++)
	...
	for (var yy = 0; yy < hh; yy++)
		{
		var data = ds_grid_get(global.pixel_grid,xx,yy);
		if data > 0 then draw_sprite(spr_note,data-1,xx*16,yy*16);
		}
	}
*/
surface_reset_target();
...
if data > 0 
then draw_sprite_part(spr_note,data-1,0,0,1,1,xx,yy)
else
	{
	gpu_set_blendmode(bm_subtract);
	draw_set_alpha(1);
